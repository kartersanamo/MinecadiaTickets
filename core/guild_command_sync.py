...
class GuildCommandSync:
    ...
    async def sync_guild_commands(
        ...
        *,
        ...
        guild_id = cls.resolve_guild_id(config_guild_id)
        if guild_id is None:
            warn("DISCORD_GUILD_ID / GUILD_ID not set — falling back to global sync only")
            synced = await cls._sync_global(bot, warn)
            info("Globally synced %s commands: %s", len(synced), cls.format_command_names(synced))
            return synced
        ...
        info(
            ...
            len(guild_cmds),
            guild_id,
            cls.format_command_names(guild_cmds),
        )
