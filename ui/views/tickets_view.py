        ...
        for category_name, category_info in list(self.tickets.items())[:5]:
            select_options = [
                discord.SelectOption(
                    label=option_name, emoji=option_info["Emoji"], description=option_info["Description"]
                )
                for option_name, option_info in category_info.items()
                if option_name != "Role"
            ]

            select = discord.ui.Select(custom_id=category_name, placeholder=category_name, options=select_options)
