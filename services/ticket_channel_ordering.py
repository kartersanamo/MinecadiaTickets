...
def _count_position(
    ...
    *,
    ...
    target_name = effective_channel_name(channel, channel, channel_name)
    target_number = ticket_numbers.get(channel.id)
    target_is_unrenamed = is_unrenamed_ticket(target_name, target_number)
    target_sort_key = ticket_sort_key(target_name)
    ...
    for ticket_channel in channels:
        ...
        resolved_name = effective_channel_name(ticket_channel, channel, channel_name)
        ticket_number = ticket_numbers.get(ticket_channel.id)
        if is_unrenamed_ticket(resolved_name, ticket_number):
            if target_is_unrenamed and resolved_name.lower() < target_name.lower():
                unrenamed_before += 1
            continue

        renamed_count += 1
        if not target_is_unrenamed and ticket_sort_key(resolved_name) < target_sort_key:
            renamed_before += 1
...
def get_ticket_position(
    ...
    *,
    ...
    category_index = _count_position(channels, channel, ticket_numbers, channel_name=channel_name)
    return category_index_to_guild_position(category, channel, category_index)
