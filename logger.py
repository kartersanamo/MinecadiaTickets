...
class CustomTimedRotatingFileHandler(TimedRotatingFileHandler):
    def __init__(
        self, filename, when="midnight", interval=1, backupCount=7, encoding=None, delay=False, utc=False, atTime=None
    ) -> None:
        super().__init__(
            filename=filename,
            when=when,
...
LOGGING_CONFIG: dict[str, Any] = {
    ...
    "formatters": {
        "file": {"format": "%(levelname)-10s  %(asctime)s  %(funcName)-15s : %(message)s", "()": ESTFormatter},
        "standard": {
    ...
    "loggers": {
        ...
        "Commands": {"handlers": ["console", "file"], "level": "INFO", "propagate": False},
        "discord": {"handlers": ["console", "file"], "level": "DEBUG", "propagate": False},
    },
}
